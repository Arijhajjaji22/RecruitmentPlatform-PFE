...
namespace App_plateforme_de_recurtement.Controllers
{
    ...
    public class TestController : ControllerBase
    {
        ...
        [HttpPost("bySubject")]
        public IActionResult GetTestsBySubject([FromBody] Test request)
        {
            // Vérifiez si l'objet request est correctement reçu
            if (request == null || string.IsNullOrEmpty(request.Subject))
            {
                return BadRequest("Subject is required.");
            }

            // Traitez ici la logique pour obtenir les tests basés sur le Subject
            var tests = _testService.GetTestsBySubject(request.Subject);
            return Ok(tests);
        }
